...
namespace API.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        {
            if(await CheckEmailExistAsync(model.Email)) return BadRequest($"An existing account is using {model.Email} email. Please try with another email address.");
        ...
        public async Task<ActionResult> ResendMailConfirmationLink(string email)
        {
        ...
        {
            var user = await _userManager.FindByEmailAsync(model.Email);
            if (user == null) return Unauthorized("This email has not been registred yet.");
